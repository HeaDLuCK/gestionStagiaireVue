...
import AddEtablissement from '../components/etablissement/AddEtablissement.vue';
import ModelEtablissement from '../components/etablissement/ModelEtablissement.vue';
import { getEtablissements } from '../services/etablissementService.js'
const dataForModel = reactive({})

const data = ref([])
onMounted(() => {
    getEtablissements().then(promise => { data.value = promise.data.data })
...
<template>
    <AddEtablissement v-if="formModel" :status="formModel" :updateStatus="updateFormModelStatus" />
    <ModelEtablissement v-if="modelStatus" :status="modelStatus" :updateStatus="updateModelStatus" :data="dataForModel" />
    <h1 class="text-3xl mt-7 font-bold text-green-700">Liste des établissements</h1>
    ...
    <div class="relative w-full flex justify-end py-3">
        <button class="transition-all duration-300 bg-green-500 p-3 text-white rounded-2xl hover:bg-green-700"
            @click="updateFormModelStatus(true)">ajouter
            ...
            <thead class="text-s font-medium text-gray-800 uppercase bg-green-500 ">
                <tr>
                    <th scope="col" class="px-6 py-3 flex cursor-pointer" @click="onClickFunc">
                        Libelle <svg-icon v-if="count" type="mdi" :path="icon" :size="15" />
                    </th>
