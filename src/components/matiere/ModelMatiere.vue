...
const onDelete = () => {
    supprimerMatiere(id.value).then(promise => { console.log(promise.data) })
    .then(()=>{router.go()})
}
const onUpdate = () => {
    object.professeurs_ids=selectedProfs.value.map(elem=>elem.id)
    modifierMatiere(id.value, object).then(promise => { console.log(promise.data); })
    .then(()=>{router.go()})
}
        ...
        <div class="w-[45%] flex flex-col gap-4  rounded-[8px] translate-x-9 translate-y-2 shadow-lg bg-white
         p-10 overflow-x-auto " @click="(e) => { e.stopPropagation(); inEdit = false }">
            ...
            <div class='item w-full'>
                <label for="libelle" class="font-bold">Libellé</label>
                <div v-if="inEdit" class="mt-2">
                    <input class="border-2 border-green-300  w-full  focus:outline-none rounded-md p-1" type="text"
