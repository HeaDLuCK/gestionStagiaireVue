...
const onDelete = () => {
    supprimerStagiaire(id.value).then(promise => { console.log(promise.data) })
        .then(() => router.go())
}
const onUpdate = () => {
    object.professeurs_ids=selectedProfs.value.map(elem=>elem.id)
    object.etablissement_id=etablissement.value.id
    modifierStagiaire(id.value, object).then(promise => { console.log(promise.data); })
    .then(() => router.go())
}
    ...
    <div class="bg-[#0000002a] fixed w-[100%] left-0 top-0 min-h-[100vh] z-10 flex items-center justify-center"
        ...
        grid grid-cols-3 p-10 overflow-x-auto " @click="(e) => { e.stopPropagation(); inEdit = false }">
            ...
            <div class='item'>
                ...
                <div v-if="inEdit">
                    <input v-model="object.nom" class="border-2 border-green-300   focus:outline-none rounded-md p-1"
            ...
            <div class='item'>
                ...
                <div v-else @dblclick="() => { inEdit = true }">{{ object.prenom }}</div>
            </div>
            <div class='item'>
                <label for="ddNassance" class="font-bold">Date de naissance</label>
                <div v-if="inEdit">
                    <input v-model="object.date_de_naissance"
            ...
            <div class='item'>
                ...
                <div v-else @dblclick="() => { inEdit = true }">{{ object.etablissement?.libelle ?? 'sans etablissement' }}</div>
            </div>
            <div class='item'>
                <label for="Login" class="font-bold">login</label>
                <div v-if="inEdit">
                    <input v-model="object.login" class="border-2 border-green-300   focus:outline-none rounded-md p-1"
                        type="text" @click="(e) => { e.stopPropagation() }">
                </div>
            ...
            <div class='item'>
                ...
                <div v-if="inEdit">
                    <input v-model="object.mot_de_passe"
